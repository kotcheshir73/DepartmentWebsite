...
namespace DepartmentService.Services
{
    public class ScheduleService : IScheduleService
    {
        ...
        public List<SemesterRecordShortViewModel> GetScheduleSemester(ScheduleBindingModel model)
        {
            ...
            var selectedRecords = _context.SemesterRecords.Include(sr => sr.Lecturer).Include(sr => sr.Classroom).Include(sr => sr.StudentGroup).AsQueryable();
        ...
        public List<ConsultationRecordShortViewModel> GetScheduleConsultation(ScheduleBindingModel model)
        {
            ...
            var selectedRecords = _context.ConsultationRecords.Include(sr => sr.Lecturer).Include(sr => sr.Classroom).Include(sr => sr.StudentGroup).AsQueryable();

            if(model.DateBegin.HasValue)
            {
                if (!string.IsNullOrEmpty(model.ClassroomId))
                {
                    selectedRecords = selectedRecords.Where(sr => sr.ClassroomId == model.ClassroomId && sr.SeasonDatesId == seasonDate.Id &&
                                                       sr.DateConsultation >= model.DateBegin.Value && sr.DateConsultation <= model.DateEnd.Value);
                }
                if (!string.IsNullOrEmpty(model.GroupName))
                {
                    selectedRecords = selectedRecords.Where(sr => sr.LessonGroup == model.GroupName && sr.SeasonDatesId == seasonDate.Id &&
                                                       sr.DateConsultation >= model.DateBegin.Value && sr.DateConsultation <= model.DateEnd.Value);
                }
            }
            else
        ...
        public ResultService ExportOffsetRecordExcel(ExportToExcelClassroomsBindingModel model)
        {
            try
            {
                ...
                for (int r = 1; r <= model.Classrooms.Count; r++)
                {
                    ...
                    #endregion
                    #region тело
                    var list = GetScheduleOffset(new ScheduleBindingModel { ClassroomId = model.Classrooms[r - 1] });
                    for (int i = 0; i < list.Count; i++)
                    {
                        excelcells = excelworksheet.get_Range(simbols[list[i].Lesson + 1] + (list[i].Day + 3 + list[i].Week * 8),
                            simbols[list[i].Lesson + 1] + (list[i].Day + 3 + list[i].Week * 8));
        ...
        public ResultService ExportExaminationRecordExcel(ExportToExcelClassroomsBindingModel model)
        {
            try
            {
                ...
                for (int r = 1; r <= model.Classrooms.Count; ++r)
                {
                    ...
                    var list = GetScheduleExamination(new ScheduleBindingModel { ClassroomId = model.Classrooms[r - 1] });
                    for (int i = 0; i < list.Count; i++)
                    {
        ...
        public ResultService ExportSemesterRecordHTML(ExportToHTMLClassroomsBindingModel model)
        {
            try
            {
                ...
                if (times == null)
                {
                    return ResultService.Error("error", "times_not_found", 404);
                }
        ...
        public ResultService ExportExaminationRecordHTML(ExportToHTMLClassroomsBindingModel model)
        {
            try
            {
                ...
                for (int i = 0; i < model.Classrooms.Count; ++i)
                {
                    ...
                    var list = GetScheduleExamination(new ScheduleBindingModel { ClassroomId = model.Classrooms[i] });
            ...
            SemesterRecordRecordBindingModel recordFirst, SemesterRecordRecordBindingModel recordSecond)
        {
            ...
            if (i < lesson.Length)
            {
                ...
                var classroom = _context.Classrooms.FirstOrDefault(c => recordFirst.LessonClassroom.Contains(c.Id) && !c.IsDeleted);
                if (classroom != null)
                {
                    recordFirst.ClassroomId = classroom.Id;
                }
            }
            ...
            //если еще остались элементы, значит есть вторая пара
            if (i < lesson.Length)
            {
                //дисциплина будет пустой
                ...
                if (i < lesson.Length)
                {
                    ...
                    var classroom = _context.Classrooms.FirstOrDefault(c => recordSecond.LessonClassroom.Contains(c.Id) && !c.IsDeleted);
                    if (classroom != null)
                    {
