...
{
    public partial class ControlEducationDirection : UserControl
    {
        [Dependency]
		...
		{
			var result = _service.GetEducationDirections(new EducationDirectionGetBindingModel { PageNumber = pageNumber, PageSize = pageSize });
			if (!result.Succeeded)
			{
		...
		{
            var form = Container.Resolve<FormEducationDirection>(
                new ParameterOverrides
                {
                    { "id", Guid.Empty }
                }
                .OnType<FormEducationDirection>());
