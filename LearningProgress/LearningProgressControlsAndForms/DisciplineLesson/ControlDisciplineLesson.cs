...
{
    public partial class ControlDisciplineLesson : UserControl
    {
        [Dependency]
        ...
        public ControlDisciplineLesson(IDisciplineLessonService service)
        {
            InitializeComponent();
            _service = service;
        ...
        {
            var form = Container.Resolve<FormDisciplineLesson>(
        ...
        private void DuplicateDisciplineLessonToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Guid id = new Guid(standartControl.GetDataGridViewSelectedRows[0].Cells[0].Value.ToString());
                var form = Container.Resolve<FormDuplicateDisciplineLesson>(
                new ParameterOverrides
                {
                    { "dlId", id }
                }
                .OnType<FormDuplicateDisciplineLesson>());
                form.Show();
            }
        }
        ...
        private void FormDisciplineLessonsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var form = Container.Resolve<FormDisciplineLessons>(
                   new ParameterOverrides
                   {
                        { "model", model }
                   }
                   .OnType<FormDisciplineLessons>());
            form.Show();
        }
    }
}