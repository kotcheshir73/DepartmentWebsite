...
{
    public partial class ControlConductedLessons : UserControl
    {
        [Dependency]
        ...
        public ControlConductedLessons(ILearningProgressProcess process, IAcademicYearService serviceAY, IEducationDirectionService serviceED, IStudentGroupService serviceSD)
        {
            InitializeComponent();
            _process = process;
        ...
        private void LoadConotrol()
        {
            if (!string.IsNullOrEmpty(comboBoxSemester.Text))
            {
                ...
                foreach (var elem in result.Result)
                {
                    ...
                    var controlDL = Container.Resolve<ControlDisciplineLessonConducted>();
